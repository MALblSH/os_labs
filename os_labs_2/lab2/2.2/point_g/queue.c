...
queue_t* queue_init(int max_count) {
	...
	err = sem_init(&q->empty_slots, PROCESS_PRIVATE, max_count);
	if (err != SUCCESS) {
		printf("queue_init: sem_init(empty_slots) failed: %s\n", strerror(err));
		free(q);
		return NULL;
	}
	err = sem_init(&q->filled_slots, PROCESS_PRIVATE, NO_ITEMS);
	if (err != SUCCESS) {
	...
	err = sem_init(&q->queue_lock, PROCESS_PRIVATE, ACCESS_ALLOWED);
	if (err != SUCCESS) {
